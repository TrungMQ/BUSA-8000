...
## PIC: Nguyen Khuat Son Tra - Student id: 48144134
...
```{r Add new columns: Total Purchase Amount}
# NEW COLUMN: TOTAL PURCHASE AMOUNT
sales <- sales %>%
  mutate(`Total Purchase Amount` = `Quantity Ordered` * `Price Each`) # add a new column for subsequent stages summary(cars)
```

```{r Export cleaned file}
...
```{r Best month in best year of sale}
month_sales <- sales %>%
  filter(Year == 2019) %>%
  group_by(Month) %>%
...
```{r Best selling pairing product}
...
## PIC: Quoc Trung Nguyen - Student id: 48299405
## a. Monthly sales trend vs Average monthly sales
...
```{r Top 10 products in best year of sale 2019}
...
```{r Top 10 products in 2020, 2021}
...
```{r Monthly order vs average monthly order}
...
## e.  Daily order trend vs daily average order
```{r Daily order vs daily average order}
...
ggplot(daily_summary, aes(x = Day, y = Total_Orders, group = 1)) +
  ...
  geom_text(aes(label = scales::comma(Total_Orders)), vjust = -0.5, hjust = 1, size = 2, angle = 45) +
...
```{r Hourly order vs hourly averaghe order}
...
ggplot(hourly_summary, aes(x = factor(Hour), y = Total_Orders)) +
  ...
  geom_text(aes(label = scales::comma(Total_Orders)), vjust = -0.5, size = 2) +
...
## TASK 4: BUILDING PREDICTIVE MODELS
## PIC: Duc Huy Tran - Student id: 48477192
...
## Two Models use for predictions (Linear & Holt-Winners)
...
```{r Accuracy for Linear Trend Model}
lt_accuracy <- accuracy(lt_forecast, test)
print(lt_accuracy)
```

## Check accuracy of Holt-Winners Model
```{r Accuracy for Holt Model}
holt_accuracy <- accuracy(holt_forecast, test)
print(holt_accuracy)
```

# Visualize Linear Model
```{r Plotting for Linear model}
...
```{r Compare accuracy metrics of models}
